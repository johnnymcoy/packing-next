...
class Painter:
    ...
    def _plot_cube(self, ax, x, y, z, dx, dy, dz, color='red',mode=2,linewidth=1,text="",fontsize=15,alpha=0.5):
    ...
    def _plot_cylinder(self, ax, x, y, z, dx, dy, dz, color='red',mode=2,text="",fontsize=10,alpha=0.2):
    ...
    def plot_box_and_items(self,title="",alpha=0.2,write_num=False,fontsize=10):
        ...
        # plot bin 
        self._plot_cube(axGlob,0, 0, 0, float(self.width), float(self.height), float(self.depth),color='black',mode=1,linewidth=2,text="")
        ...
        for item in self.items:
            ...
            if item.typeof == 'cube':
                 # plot item of cube
                self._plot_cube(axGlob, float(x), float(y), float(z), float(w),float(h),float(d),color=color,mode=2,text=text,fontsize=fontsize,alpha=alpha)
            elif item.typeof == 'cylinder':
                # plot item of cylinder
                self._plot_cylinder(axGlob, float(x), float(y), float(z), float(w),float(h),float(d),color=color,mode=2,text=text,fontsize=fontsize,alpha=alpha)
